...
# 创建数据集
dataset = Unet_Dataset(train_dir_p0, 'p0', train_dir_ua, 'ua_true')
# 数据集划分的准备工作
valid_size = 0.1
num_train = len(dataset)
indices = list(range(num_train))  # 变成一个数据集长度的列表
split = int(np.floor(valid_size * num_train))   # 取一个分割点,就是一个整数
train_idx, valid_idx = indices[split:], indices[:split]     # train要split后面的，valid要split前面的（这个好哎）
print("{} mat images for train and {} mat images for val".format(len(train_idx),len(valid_idx)))
...
for i in range(epoch):
    ...
    with torch.no_grad():
        ...
        valid_loss = total_val_loss / len(valid_idx)
