...
from nets.backbones.mobilenetv2 import mobilenetv2
...
class _PSPModule(nn.Module):
    ...
    def forward(self, features):
        ...
        for stage in self.stages:
            psp_channel = F.interpolate(stage(features), size=(h, w), mode='bilinear', align_corners=True)
            pyramids.extend([psp_channel])
...
if __name__ == '__main__':
    model = PSPNet(num_classes=3, backbone="mobilenet", downsample_factor=16, pretrained=False,
                   aux_branch=False)
    print(model)